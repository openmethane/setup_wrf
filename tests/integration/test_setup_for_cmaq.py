...
def test_setup_for_cmaq(tmpdir, root_dir, wrf_run, file_regression, request, data_regression, compare_dataset):
    ...
    file_regression.check(open(mcip_dir / "2022-07-22" / "d01" / "GRIDDESC").read(), basename=f"{request.node.name}_griddesc")

    # Compare the structure of a select set of files
    compare_dataset(cmaq_dir / "template_bcon_profile_CH4only_d01.nc", basename=f"{request.node.name}_bcon")
