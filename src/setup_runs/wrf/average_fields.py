...
def average_fields(inFile, outFile, outputTime):
    ...
    for name, var in src.variables.items():
        if '_FillValue' in var.ncattrs():
            ...
            fill_value = None
        ##
        variables[name] = {'dtype': var.dtype, 'dimensions': var.dimensions,
                           'fill_value': fill_value}
        variables[name]['dimlens'] = numpy.array(
            [dimensions[dim] if dimensions[dim] is not None else 1 for dim in
    ...
    for name in variables.keys():
        if name != 'Times':
            iTime = [idim for idim, dim in enumerate(variables[name]['dimensions']) if
                     dim == 'Time']
            if len(iTime) == 0:
                ...
                raise RuntimeError(
                    "Multiple matches for the Time dimension for variable {}".format(name))
    src.close()
    ## create an output file
    trg = netCDF4.Dataset(outFile, mode='w')
    for dim in dimensions.keys():
        trg.createDimension(dim, dimensions[dim])
    ##
    trg.setncatts(attributes['global'])
    ##
    for name in variables.keys():
        ## set up the variables
        trg.createVariable(name,
        ...
        ## write out the data
        if name == 'Times':
            trg.variables[name][:] = numpy.array([c for c in outputTime], dtype='|S1').reshape(
                variables[name]['dimlens'])
        else:
...
if __name__ == "__main__":
    ...
    average_fields(inFile, outFile, outputTime)
